...
func Run_migrations() {
	...
	var host = util.GetEnv("DB_HOST", "")
	var user = util.GetEnv("DB_USER", "")
	var password = util.GetEnv("DB_PASS", "")
	var database = util.GetEnv("DB_DATABASE", "")
	ConnectionString :=
		"host=" + host + " user=" + user + " password=" + password + " dbname=" + database + " sslmode=disable"
